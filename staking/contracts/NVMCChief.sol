...
/*
    NVMCChief contract for Alyra DeFi #4
*/
contract NVMCChief is Ownable {
    using SafeMath for uint256;
    NVMC contractNVMC = new NVMC();
    ...
    IERC20 public depositToken; // eg. LP token
    ...
    external onlyOwner {
        require(block.timestamp > rewardPeriodEndTimestamp, "NVMCChief: can't add rewards before period finished");
        updateRewards();
        ...
        require(rewardToken.transferFrom(msg.sender, address(this), _rewardsAmount), "NVMCChief: transfer failed");
    ...
    external {
        ...
        if (user.deposited > 0) {
            ...
            require(rewardToken.transfer(msg.sender, pending), "NVMCChief: transfer failed");
        ...
        require(depositToken.transferFrom(msg.sender, address(this), _amount), "NVMCChief: transferFrom failed");
    ...
    external {
        UserInfo storage user = users[msg.sender];
        require(user.deposited >= _amount, "NVMCChief: balance not enough");
        updateRewards();
        // Send reward for previous deposits
        uint256 pending = user.deposited.mul(accumulatedRewardPerShare).div(1e12).div(1e7).sub(user.rewardsAlreadyConsidered);
        require(rewardToken.transfer(msg.sender, pending), "NVMCChief: reward transfer failed");
        emit ClaimReward(msg.sender, pending);
        user.deposited = user.deposited.sub(_amount);
        totalStaked = totalStaked.sub(_amount);
        user.rewardsAlreadyConsidered = user.deposited.mul(accumulatedRewardPerShare).div(1e12).div(1e7);
        require(depositToken.transfer(msg.sender, _amount), "NVMCChief: deposit withdrawal failed");
    ...
    external onlyOwner {
        ...
        if (depositTokenBalance > totalStaked) {
            ...
            require(depositToken.transfer(msg.sender, amount), "NVMCChief: transfer failed");
            emit Skim(amount);
        }
    }
