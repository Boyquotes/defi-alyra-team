...
import NVMCStakeContract from "./contracts/NVMCStake.json";
...
function App() {
  ...
  const [nvmcStakeContract, setNvmcStakeContract] = useState(undefined);
  ...
  async function initConnection() {
    try {
      ...
      const deployedNetwork = NVMCStakeContract.networks[networkId];
      const instance = new web3.eth.Contract(
        NVMCStakeContract.abi,
  ...
  useEffect(() => {
    ...
    if (typeof web3 !== 'undefined'
      ...
      && typeof nvmcStakeContract !== 'undefined'
      && typeof depositTokenContract !== 'undefined'
      && typeof rewardTokenContract !== 'undefined') {
        load();
      }
  }, [web3, accounts, nvmcStakeContract, depositTokenContract, rewardTokenContract]) // eslint-disable-line react-hooks/exhaustive-deps


  async function refreshUserDetails() {
    setIsGlobalLoading(true);
    let res = await nvmcStakeContract.methods.getFrontendView().call({ from: accounts[0] });
  ...
  return (
    <div className="outerApp">
      <BlockchainContext.Provider value={{web3, accounts, nvmcStakeContract, rewardTokenContract, depositTokenContract}}>
