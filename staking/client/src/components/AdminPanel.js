...
export default function AdminPanel() {
    ...
    const { web3, accounts, nvmcStakeContract, rewardTokenContract } = blockchainContext;
    ...
    async function addRewards() {
        ...
        try {
            ...
            await rewardTokenContract.methods.approve(nvmcStakeContract.options.address, amount.toString()).send({ from: accounts[0] });
            toast.dismiss();
            toast.info('Please approve transaction 2 of 2 (add rewards)...', {position: 'top-left', autoClose: false});
            await nvmcStakeContract.methods.addRewards(amount.toString(), days).send({ from: accounts[0] });
        } finally {
            toast.dismiss();
        }
