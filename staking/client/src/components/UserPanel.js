...
export default function UserPanel() {
    ...
    const { web3, accounts, nvmcStakeContract, depositTokenContract } = blockchainContext;
    ...
    async function deposit() {
        ...
        try {
            ...
            await depositTokenContract.methods.approve(nvmcStakeContract.options.address, amount.toString()).send({ from: accounts[0] });
            toast.dismiss();
            toast.info('Please approve transaction 2 of 2 (staking)...', {position: 'top-left', autoClose: false});
            await nvmcStakeContract.methods.deposit(amount).send({ from: accounts[0] });
        } finally {
            toast.dismiss();
        }
        setInputStake("");
    ...
    async function withdraw() {
        ...
        try {
            await nvmcStakeContract.methods.withdraw(amount).send({ from: accounts[0] });
        } finally {
            toast.dismiss();
        }
        setInputUnstake("");
    ...
    async function claim() {
        ...
        toast.info('Please approve transaction...', {position: 'top-left', autoClose: false});
        try {
            await nvmcStakeContract.methods.claim().send({ from: accounts[0] });
        } finally {
            toast.dismiss();
        }
        await refreshUserDetails();
