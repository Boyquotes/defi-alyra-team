...
contract("NVMC", async accounts => {
    ...
    it("should calculate the parameters correctly", async () => {
        ...
        const depositTokenAddr = await nvmc.depositToken.call();
        const rewardTokenAddr = await nvmc.rewardToken.call();

        let rewardAmount = web3.utils.toWei("300");
        let days = 30;

        const rewardToken = await MockERC20.at(rewardTokenAddr);
        await rewardToken.approve(nvmc.address, rewardAmount, defaultOptions);
        
        // Add staking rewards
        await nvmc.addRewards(rewardAmount, days, defaultOptions);
        let startingTime = await getTime();

        // Verify rewards-per-second calculation is correct
        let contractRps = await nvmc.rewardPerSecond.call(defaultOptions);
        ...
        let contractEndTime = await nvmc.rewardPeriodEndTimestamp.call(defaultOptions);
    ...
    it("should calculate and distribute rewards correctly to one person across multiple campaigns and multiple actions", async () => {
        ...
        await rewardToken.approve(nvmc.address, rewardAmount, defaultOptions);
        await nvmc.addRewards(rewardAmount, days, defaultOptions);
        let prevContractRewardTokenBalance = await rewardToken.balanceOf(nvmc.address, { from: accounts[1] });
        ...
        await depositToken.approve(nvmc.address, depositAmount, { from: accounts[1] });
        await nvmc.deposit(depositAmount, { from: accounts[1] });
        let initialReward = await nvmc.pendingRewards.call(accounts[1], { from: accounts[1] });
        ...
        let contractPendingReward = await nvmc.pendingRewards.call(accounts[1], { from: accounts[1] });
        ...
        await nvmc.claim({ from: accounts[1] });
        ...
        let contractNewRewardBalance = await rewardToken.balanceOf(nvmc.address, { from: accounts[1] });
        ...
        await nvmc.withdraw(depositAmount, { from: accounts[1] });
        ...
        let newDays = 6;
        await rewardToken.approve(nvmc.address, newRewardAmount, defaultOptions);
        await nvmc.addRewards(newRewardAmount, newDays, defaultOptions);
        let newSecsToAdvance = 60*60*24*3;
        await timeMachine.advanceTimeAndBlock(newSecsToAdvance);
        let newContractRps = await nvmc.rewardPerSecond.call(defaultOptions);
        // Test pendingRewards()
        ...
        contractPendingReward = await nvmc.pendingRewards.call(accounts[1], { from: accounts[1] });
        ...
        await nvmc.withdraw(withdrawAmount.toString(), { from: accounts[1] });
    ...
    it("should calculate and distribute rewards correctly to multiple nvmcs + skim functionality", async () => {
        const nvmc = await NVMC.deployed();
        const depositTokenAddr = await nvmc.depositToken.call();
        const rewardTokenAddr = await nvmc.rewardToken.call();
        ...
        let depositAmount = web3.utils.toWei("10");
        await depositToken.transfer(nvmc.address, depositAmount, { from: accounts[1] });
        ...
        await depositToken.approve(nvmc.address, depositAmount2, { from: accounts[2] });
        await nvmc.deposit(depositAmount2, { from: accounts[2] });
        ...
        await nvmc.withdraw(depositAmount.toString(), { from: accounts[1] });
        let user1EndingTime = await getTime();
        await nvmc.withdraw(depositAmount2.toString(), { from: accounts[2] });
        ...
        await nvmc.skim({from: accounts[0] });
    ...
    it("should not let user withdraw more than staked", async () => {
        ...
        try {
            await nvmc.withdraw(withdrawAmount, { from: accounts[1] });;
            throw null;
        }
        catch (error) {
    ...
    it("should limit sensitive functions only to owner", async () => {
        ...
        try {
            await nvmc.addRewards(rewardAmount, days, {from: accounts[1]});
            throw null;
        }
        catch (error) {
            assert(error, "Expected an error but did not get one");
        ...
        try {
            await nvmc.skim( {from: accounts[1]});
            throw null;
        }
        catch (error) {
            assert(error, "Expected an error but did not get one");
